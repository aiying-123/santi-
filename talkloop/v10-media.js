...
(function(){
  ...
  const FAMILIES=[
    ...
    {key:'plan', re:/time|clock|calendar|tomorrow|tonight|free|plan|invite|dinner|appointment|schedule/i, scenes:['appointment','invite'], en:[URL.socialA,URL.socialB,URL.workA], jp:[URL.jpA,URL.jpC,URL.jpD], window:[0,22]},
    {key:'social', re:/.*/, scenes:[], en:[URL.socialA,URL.socialB,URL.socialC], jp:[URL.jpA,URL.jpC,URL.jpD], window:[0,20]}
  ];

  function compactText(item){return [item.action,item.func,item.en,item.jp].filter(Boolean).join(' ')}
  function familyFor(item){
    const text=compactText(item), scene=item.scene&&item.scene.id;
    for(const f of FAMILIES){
      if(f.key==='social') continue;
      if(f.re.test(text)) return f;
    }
    for(const f of FAMILIES){
      if(f.key==='social') continue;
      if(f.scenes.includes(scene)) return f;
    }
    return FAMILIES[FAMILIES.length-1];
  }
  function poolFor(item,l){
    const f=familyFor(item);
    const primary=l==='jp'?f.jp:f.en;
    const fallback=l==='jp'?[URL.jpA,URL.jpB,URL.jpC,URL.jpD]:[URL.socialA,URL.retailA,URL.travelA,URL.workA];
    return [...new Set([...primary,...fallback].filter(Boolean))];
  }
  function mappingFor(item,l){
    const f=familyFor(item), pool=poolFor(item,l);
    return {expression:item.id,scene:item.scene.id,intent:item.func||'',action:item.action||'',family:f.key,video_pool:pool,window:f.window,language:l};
  }
  const MEDIA_INDEX={};
  ITEMS.forEach(item=>{MEDIA_INDEX[item.id]={en:mappingFor(item,'en'),jp:mappingFor(item,'jp')}});
  window.TALKLOOP_MEDIA_INDEX=MEDIA_INDEX;
  window.TALKLOOP_MEDIA_SCHEMA={version:'10.0.0',mapped:Object.keys(MEDIA_INDEX).length,families:FAMILIES.map(x=>x.key)};

  window.pools=function(item,l){return poolFor(item,l)};
  window.prepareNext=function(item,l){
    const p=poolFor(item,l); if(!p.length) return;
    const visit=(st.vis[item.id+'|'+l]||0);
    const seed=hash(item.id+'|'+l+'|warm|'+visit);
    for(let i=0;i<Math.min(3,p.length);i++) warm(p[(seed+i)%p.length]);
  };

  window.remix=function(item,v,ph,l=lang,reason='auto',shadeId){
    stopMedia(v);
    const map=mappingFor(item,l), pool=map.video_pool, scene=v.closest('.scene'), shade=shadeId?$(shadeId):scene.querySelector('.mediaShade');
    const k=item.id+'|'+l; st.vis[k]=(st.vis[k]||0)+1; save();
    const visit=st.vis[k], seed=hash(k+'|'+visit+'|'+reason+'|v10');
    const src=pool.length?pool[seed%pool.length]:'';
    const minT=map.window[0], maxT=map.window[1];
    const clipLen=4+(seed%4);
    const desired=minT + (seed % Math.max(1,Math.floor(maxT-minT+1)));
    ph.style.backgroundImage=`url('${item.scene.img}')`; ph.style.display='block'; ph.style.opacity='1';
    scene.classList.remove('video-ready'); scene.classList.add('loading'); if(shade)shade.style.display='grid';
    if(!src){scene.classList.remove('loading');return}
    warm(src);
    const fallback=()=>{scene.classList.remove('loading');scene.classList.remove('video-ready');v.style.opacity='0';ph.style.opacity='1';if(shade)shade.style.display='none'};
    v.src=src; v.preload='auto'; v.muted=true; v.playsInline=true; v.onerror=fallback;
    v.oncanplay=()=>{
      const dur=isFinite(v.duration)?v.duration:0;
      const maxStart=Math.max(0,dur-clipLen-.2), start=Math.max(0,Math.min(desired,maxStart));
      v._s=start; v._len=Math.min(7,Math.max(4,Math.min(clipLen,dur||clipLen)));
      try{v.currentTime=start}catch(e){}
      scene.classList.remove('loading'); scene.classList.add('video-ready'); if(shade)shade.style.display='none';
      v.style.opacity='1'; ph.style.opacity='0'; v.play().catch(()=>{}); window.prepareNext(item,l);
    };
    v.ontimeupdate=()=>{
      if(v.currentTime>=(v._s||0)+(v._len||clipLen)-.08){
        window.remix(item,v,ph,l,'segment',shadeId);
      }
    };
    v.load(); mediaState.set(v,{src,map});
  };

  const oldAudit=window.renderAudit;
  window.renderAudit=function(){
    if(oldAudit) oldAudit();
    const node=$('audit'); if(!node) return;
    const mapped=Object.keys(MEDIA_INDEX).length;
    const actionCount=new Set(ITEMS.map(x=>x.action).filter(Boolean)).size;
    const familyCount=new Set(ITEMS.map(x=>familyFor(x).key)).size;
    node.insertAdjacentHTML('beforeend',`<div class="tip" style="margin-top:10px"><b>视频映射层 v10</b><br>表达 → 意图 → 动作 → 视频池 已映射 ${mapped}/${ITEMS.length} 条；动作标签 ${actionCount} 类；视觉动作族 ${familyCount} 类。高频不熟/模糊表达会在复训时增加素材与时间片变化。</div>`);
  };
