...
const ACTION=[
 ...
 {k:'payment',re:/card|cash|visa|terminal|receipt|refund|fee|charge|pay|checkout|bank|transfer|withdraw|deposit/i,en:[PX.retail1,PX.retail2,MX.phoneDiscuss],jp:JP_WOMEN},
 {k:'order',re:/coffee|cup|menu|food|drink|milk|water|bag|restaurant|cafe|order|dish|pizza/i,en:[PX.retail2,PX.retail3,MX.cafeWomen2,MX.coupleChat],jp:JP_WOMEN},
 {k:'shopping',re:/size|color|try|shirt|item|shop|return|exchange|price|gift|package/i,en:[PX.retail3,PX.retail1,MX.phoneDiscuss],jp:JP_WOMEN},
 {k:'travel',re:/train|station|platform|flight|airport|gate|boarding|luggage|passport|map|direction|hotel|taxi|bus/i,en:[PX.travel1,PX.hotel1,MX.walkTalk,MX.phoneDiscuss],jp:JP_WOMEN},
 {k:'phone',re:/phone|call|text|message|signal|battery|email|contact|linkedin/i,en:[MX.phoneConcern,MX.callMan,MX.phoneDiscuss,PX.work1],jp:JP_WOMEN},
 {k:'work',re:/work|office|priority|deadline|project|meeting|supervisor|manager|task|team|laptop|assignment|class|teacher/i,en:[MX.meeting,MX.coworkers,MX.meetingEnd,MX.partners,MX.officeWalk,MX.classTalk,PX.work1],jp:JP_WOMEN},
 {k:'health',re:/doctor|health|pain|sick|medicine|pharmacy|ambulance|headache|fever/i,en:[PX.health1,MX.phoneConcern,MX.coworkers],jp:JP_WOMEN},
 {k:'gym',re:/gym|workout|machine|weight|class|exercise|locker|shower|towel|training/i,en:[MX.gym,MX.coworkers],jp:JP_WOMEN},
 {k:'home',re:/home|roommate|trash|dishes|router|cook|bed|neighbor|noise|building/i,en:[MX.coupleChat,MX.walkTalk,MX.phoneDiscuss],jp:JP_WOMEN},
 {k:'conflict',re:/complaint|problem|wrong|damaged|blocked|stolen|lost|argue|mistake|sorry/i,en:[MX.argue,MX.phoneConcern,MX.coworkers],jp:JP_WOMEN},
...
function pool(item,l){const a=family(item);if(l==='jp')return [...new Set(JP_WOMEN.filter(Boolean))];return [...new Set(a.en.filter(Boolean))]}
window.TALKLOOP_MEDIA_SOURCES={version:'11.1.0',providers:['Pexels','Mixkit'],actionFamilies:ACTION.map(x=>x.k),jpWomenPool:JP_WOMEN.length};
window.pools=pool;
...
window.remix=function(item,v,ph,l=lang,reason='auto',shadeId){
  ...
  const trySource=()=>{
    ...
    v.onloadeddata=onReady;v.oncanplay=onReady;v.onerror=trySource;v._tlTimer=setTimeout(()=>{if(!ready)trySource()},950);v.ontimeupdate=()=>{if(ready&&v.currentTime>=(v._s||0)+(v._len||5)-.08){window.remix(item,v,ph,l,'segment',shadeId)}};try{v.load()}catch(e){trySource()}
